...
Para facilitar a identificação visual dos diversos tipos de avaliações encontradas, uma estrutura foi esquematizada da seguinte forma: na base foi adicionada a avaliação, objeto de estudo dessa seção; no primeiro nível ficaram os aspectos pelos quais a avaliação foi organizada; e, por fim, no segundo nível aparecem os tipos de avaliação que foram identificados para cada aspecto. A \refFig{fig:organogfunc} apresenta a estrutura completa. 

\textbf{(--> TROQUEI ORGANOGRAMA por estrutura, pois acho que nos compromete menos. É preciso revisar para ver se não ficou nenhum inconsistência com esta troca.)} \textcolor{blue}{ok, alterado no documento.}
...
\end{tikzpicture}
    \caption{Estrutura que representa os aspectos e os tipos de avaliação}
